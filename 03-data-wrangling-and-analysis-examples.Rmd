---
title: "Data Wrangling and Analysis Examples"
...
Let's import our data using `read_csv`. Note that the NHANES data is in the data directory so we need to include that. Then, let's use clean_names to make the variable names easy to work with.
...
```{r}
nhanes %>% 
  filter(smoke_now == "Yes" & (health_gen == "Good" | health_gen == "Vgood" | health_gen == "Excellent")) %>%
  select(smoke_now, health_gen)
```
...
We can use `group_by` with multiple groups. Let's calculate the mean number of active days for each year the survey was done and for smoking status.

```{r}
nhanes %>% 
  group_by(survey_yr, smoke_now, education) %>% 
  summarize(mean_active_days = mean(phys_active_days,
                                    na.rm = TRUE))
```
...
```{r}
nhanes %>% 
  tabyl(age_decade, smoke_now, education) %>% 
  adorn_totals(c("row", "col")) %>% 
  adorn_percentages() %>% 
  adorn_pct_formatting() %>% 
  adorn_ns() %>% 
